...
const PROFILS: MembreProfil[] = [
  ...
  },
  {
    ...
    prenom: 'Marie',
    matricule: 'CAYA-002',
    role: RoleMembre.TRESORIER,
    parts: 1,
    cotisationMensuelle: 100_000,
    bouffeMois: 1,
  },
  {
    nom: 'Atangana',
...
async function run() {
  try {
    ...
    if (!typeTontine) {
      typeTontine = await tontineTypeService.create({
        code: 'MIXTE',
        libelle: 'Tontine Mixte (Cotisation + Épargne)',
      });
    }
    ...
    const reunionsTriees = reunions.sort(
      (a: any, b: any) => new Date(a.dateReunion).getTime() - new Date(b.dateReunion).getTime()
    );
    ...
    for (const m of membresActifs) {
      ...
      await payerCotisation(
        r1.id,
        m.id,
        profil.cotisationMensuelle,
        `Cotisation Oct ${profil.prenom}`,
        valideurId
      );

      // Pot
      await payerTransaction(
        r1.id,
        m.id,
        TypeTransaction.POT,
        POT_MONTANT,
        `Pot Oct ${profil.prenom}`,
        valideurId
      );
      ...
      await payerTransaction(
        r1.id,
        m.id,
        TypeTransaction.SECOURS,
        SECOURS_MONTANT,
        `Secours Initial ${profil.prenom}`,
        valideurId
      );

      success(
        `${profil.prenom} ${profil.nom}: Cotis=${profil.cotisationMensuelle.toLocaleString()}, Pot=3k, Secours=50k`
      );
    }
    ...
    await payerTransaction(
      ...
      totalCotisationsMois1,
      'BOUFFE: Marie reçoit les cotisations Mois 1',
      valideurId
    );
    ...
    await payerTransaction(
      r1.id,
      francoise!.id,
      TypeTransaction.EPARGNE,
      500_000,
      'Épargne libre Françoise',
      valideurId
    );
    ...
    await payerCotisation(
      r3.id,
      samuel!.id,
      profilMap['CAYA-007'].cotisationMensuelle,
      'Rattrapage Cotisation Nov Samuel',
      valideurId
    );
    await payerTransaction(
      r3.id,
    ...
    await payerTransaction(
      ...
      jean!.id,
      TypeTransaction.COTISATION,
      totalCotisationsMois3,
      'BOUFFE: Jean reçoit cotisations Mois 3',
      valideurId
    );
    ...
    await payerTransaction(
      ...
      paul!.id,
      TypeTransaction.COTISATION,
      totalCotisationsMois5,
      'BOUFFE: Paul reçoit cotisations Mois 5',
      valideurId
    );
    ...
    success(
      `Alice: Remboursement TOTAL 100k + ${interetAlice.toLocaleString()} intérêts (Prêt soldé ✔)`
    );
    ...
    success(
      `Paul: Remboursement FINAL 100k + ${interetPaulM5.toLocaleString()} intérêts (Prêt soldé ✔)`
    );
    ...
    for (let mois = 5; mois < 12; mois++) {
      ...
      for (const m of membresActifs) {
        ...
        await payerCotisation(
          ...
          m.id,
          profil.cotisationMensuelle,
          `Cotisation M${numMois} ${profil.prenom}`,
          valideurId
        );
        await payerTransaction(
          r.id,
    ...
    try {
      ...
      for (const c of cassations) {
        ...
        if (m) {
          info(
            `${m.matricule}: Brut=${c.montantBrut}, Déductions=${c.deductions}, Net=${c.montantNet}`
          );
        }
      }
