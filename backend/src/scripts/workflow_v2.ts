...
// Données des membres
const MEMBRES_DATA = [
  { nom: 'NKOULOU', prenom: 'Paul', tel: `69010${TIMESTAMP}1`, matricule: `${NOM_COURT_TONTINE}-001`, role: 'PRESIDENT' },
  { nom: 'MBARGA', prenom: 'Jeanne', tel: `69010${TIMESTAMP}2`, matricule: `${NOM_COURT_TONTINE}-002`, role: 'TRESORIER' },
  { nom: 'ATANGANA', prenom: 'Michel', tel: `69010${TIMESTAMP}3`, matricule: `${NOM_COURT_TONTINE}-003`, role: 'MEMBRE' },
  { nom: 'ESSOMBA', prenom: 'Sophie', tel: `69010${TIMESTAMP}4`, matricule: `${NOM_COURT_TONTINE}-004`, role: 'MEMBRE' },
];
...
async function main() {
  ...
  try {
    ...
    await queryRunner.query(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `, [tontineId, `Tontine Solidarité ${NOM_COURT_TONTINE}`, NOM_COURT_TONTINE, 2026, 'Ensemble pour réussir', 'ACTIVE', tontineType.id]);
    ...
    log(`✅ Exercice créé: "Exercice 2026" (${dateDebut.toLocaleDateString('fr-FR')} - ${dateFin.toLocaleDateString('fr-FR')})`);
    ...
    for (let i = 0; i < adhesions.length; i++) {
      ...
      exerciceMembres.push({ id: emId, adhesionId: adhesion.id, ...adhesion, roleExercice, ordre: i + 1 });
    ...
    await queryRunner.query(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `, [typePenaliteAbsence, 'ABSENCE_WF', 'Absence à la réunion', 'Pénalité pour absence non justifiée', 1000]);
    ...
    await queryRunner.query(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `, [typeSecoursDeces, 'DECES_PARENT_WF', 'Décès d\'un parent', 'Aide en cas de décès d\'un parent proche', 30000]);
    ...
    for (let mois = 1; mois <= CONFIG.DUREE_EXERCICE; mois++) {
      separator(`📅 RÉUNION ${mois} - ${new Date(2026, mois, 15).toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' })}`);
      ...
      await queryRunner.query(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `, [reunionId, exerciceId, mois, dateReunion, '14:00', mois % 2 === 0 ? 'Chez Paul' : 'Chez Jeanne', 'CLOTUREE', dateReunion, dateReunion, exerciceMembres[mois % 4].id, exerciceMembres[0].id]);
      ...
      for (const em of exerciceMembres) {
        ...
        await queryRunner.query(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `, [cotisationId, reunionId, em.id, CONFIG.COTISATION_MENSUELLE, CONFIG.COTISATION_MENSUELLE, 0, 'PAYE']);
      ...
      await queryRunner.query(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `, [distributionId, reunionId, beneficiaire.id, totalCotisations, totalCotisations, 'DISTRIBUEE', dateReunion]);
      ...
      for (const em of exerciceMembres) {
        ...
        await queryRunner.query(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `, [epargneId, reunionId, em.id, CONFIG.EPARGNE_MENSUELLE, CONFIG.EPARGNE_MENSUELLE, 0, 'PAYE']);
        epargnesParMembre[em.id] += CONFIG.EPARGNE_MENSUELLE;
        log(`      ✓ ${em.prenom}: ${CONFIG.EPARGNE_MENSUELLE.toLocaleString()} FCFA (total: ${epargnesParMembre[em.id].toLocaleString()} FCFA)`);
      }
      ...
      log(`      Total collecté: ${totalPot.toLocaleString()} FCFA (consommé pour rafraîchissements)`);
      ...
      if (mois === 2) {
        ...
        await queryRunner.query(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `, [pretMichelId, michel.id, montantPret, montantPret, CONFIG.TAUX_INTERET_PRET, montantPret * CONFIG.TAUX_INTERET_PRET / 100, montantTotal, 0, montantTotal, 'APPROUVE', dateReunion, dateReunion, new Date(2026, 5, 15), exerciceMembres[0].id]);
        caissePrets -= montantPret;
        log(`      💸 Prêt accordé à Michel: ${montantPret.toLocaleString()} FCFA à ${CONFIG.TAUX_INTERET_PRET}%`);
      ...
      if (mois === 3) {
        ...
        await queryRunner.query(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `, [evenementId, jeanne.id, typeSecoursDeces, new Date(2026, 3, 10), 'Décès du père de Jeanne', montantSecours, montantSecours, 'APPROUVE', dateReunion, exerciceMembres[0].id]);
        caisseSecours -= montantSecours;
        log(`      🕯️ Décès du père de Jeanne - Aide accordée: ${montantSecours.toLocaleString()} FCFA`);
      ...
      if (mois === 3) {
        ...
        await queryRunner.query(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `, [penaliteAbsenceId, sophie.id, typePenaliteAbsence, reunionId, 1000, 'Absence non justifiée à la réunion #3', 'EN_ATTENTE', dateReunion, exerciceMembres[0].id]);
        ...
        await queryRunner.query(`
          INSERT INTO penalite (id, exercice_membre_id, type_penalite_id, reunion_id, montant, motif, statut, applique_le, applique_par_exercice_membre_id)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `, [penaliteRetardId, michel.id, typePenaliteRetard, reunionId, 500, 'Retard de 45 minutes à la réunion #3', 'EN_ATTENTE', dateReunion, exerciceMembres[0].id]);
      ...
      if (mois === 4 && pretMichelId) {
        ...
        await queryRunner.query(`
          ...
          VALUES (?, ?, ?, ?, ?)
        `, [remboursementId, pretMichelId, montantRemboursement, dateReunion, 'Remboursement intégral']);
    ...
    for (const em of exerciceMembres) {
      ...
      await queryRunner.query(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `, [cassationId, em.id, montantEpargne, montantEpargne, 'DISTRIBUEE', new Date(), 'Cassation fin d\'exercice 2026']);
