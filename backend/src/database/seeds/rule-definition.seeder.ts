...
import {
  ...
  CategorieRegle,
  TypeValeurRegle,
} from '../../modules/tontines/entities/rule-definition.entity';
...
export const seedRuleDefinitions = async (): Promise<void> => {
  ...
  const rules = [
    ...
    {
      cle: 'REPARTITION_BENEFICES',
      libelle: 'Mode de répartition des dividendes',
      categorie: CategorieRegle.GLOBAL,
      typeValeur: TypeValeurRegle.TEXTE,
      valeurDefaut: 'PRORATA', // ou EGALITAIRE
      valeurMin: null,
      unite: null,
      description: 'Détermine si les bénéfices (intérêts prêts, pénalités) sont répartis EGALITAIRE ou au PRORATA de l\'épargne.',
      estObligatoire: true,
      estModifiableParTontine: true,
      estModifiableParExercice: true,
      ordreAffichage: 60,
