...
export class CassationService {
    ...
    async calculerPourExercice(exerciceId: string): Promise<CassationResponseDto[]> {
        ...
        if (existingCassations.length > 0) {
            throw new BadRequestError('Les cassations ont déjà été calculées pour cet exercice. Supprimez-les avant de recalculer.');
        }
        ...
        for (const membre of membres) {
            // 1. Calculer l'épargne totale accumulée
            ...
            // 2a. Prêts non remboursés (capital restant)
            ...
            const capitalRestantPrets = [...pretsEnCours, ...pretsDefaut]
                .reduce((sum, p) => sum + Number(p.capitalRestant), 0);
            if (capitalRestantPrets > 0) {
                detailDeductions['prets_non_rembourses'] = capitalRestantPrets;
                totalDeductions += capitalRestantPrets;
            }

            // 2b. Pénalités impayées (statut EN_ATTENTE = non payées)
            ...
            const totalPenalites = penalitesImpayees
            ...
            // 3. Montant net
            const montantNet = Math.max(0, epargneTotale - totalDeductions);
    ...
    async getSummary(exerciceId: string): Promise<CassationSummaryDto> {
        ...
        cassations.forEach(c => {
            summary.totalMontantBrut += Number(c.montantBrut);
            summary.totalDeductions += Number(c.deductions);
            summary.totalMontantNet += Number(c.montantNet);
    ...
    async resetExercice(exerciceId: string): Promise<void> {
        ...
        if (distributed.length > 0) {
            throw new BadRequestError('Impossible de réinitialiser: des cassations ont déjà été distribuées');
        }
    ...
    private formatResponse(cassation: Cassation, membre?: ExerciceMembre): CassationResponseDto {
        ...
        return {
            ...
            montantBrut: Number(cassation.montantBrut),
            deductions: Number(cassation.deductions),
            detailDeductions: cassation.detailDeductions,
            montantNet: Number(cassation.montantNet),
