...
export class MemberDashboardService {
    ...
    private get inscriptionDueRepo(): Repository<InscriptionDueExercice> { return AppDataSource.getRepository(InscriptionDueExercice); }
    ...
    async getMemberStats(exerciceMembreId: string): Promise<MemberDashboardResponseDto> {
        ...
        const [soldes, dettes, pretDetail, beneficiaireInfo, prochaineReunion, activiteRecente] = await Promise.all([
        ...
        return {
            ...
            prochaineReunion: prochaineReunion ? {
                ...
                date: prochaineReunion.dateReunion instanceof Date
     ...
     */
    private async calculerEcheancierPret(exerciceMembreId: string): Promise<EcheancierPretDetail | null> {
        ...
        const montantProchaineEcheance = Math.round((montantCapitalMensuel + montantInteretMensuel) * 100) / 100;
     ...
     */
    private async verifierBeneficiaire(exerciceMembreId: string, exerciceId: string): Promise<BeneficiaireInfo> {
     ...
     */
    private async getActiviteRecente(exerciceMembreId: string): Promise<MemberDashboardResponseDto['activiteRecente']> {
