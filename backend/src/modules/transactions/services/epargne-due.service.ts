...
import { DueFiltersDto, UpdateDuePaymentDto } from '../dto/dues.dto';
...
export class EpargneDueService {
  ...
  private get epargneDueRepository(): Repository<EpargneDueMensuelle> {
    if (!this._epargneDueRepo)
      this._epargneDueRepo = AppDataSource.getRepository(EpargneDueMensuelle);
  ...
  ): Promise<EpargneDueResponseDto | null> {
