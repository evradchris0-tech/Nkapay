...
export class TontineTypeService {
  ...
  async create(dto: CreateTontineTypeDto): Promise<TontineTypeResponseDto> {
    ...
    const existing = await tontineTypeRepository.findOne({
      where: { code: dto.code },
    });
    if (existing) {
    ...
    const tontineType = tontineTypeRepository.create({
      code: dto.code,
      libelle: dto.libelle,
    ...
    const saved = await tontineTypeRepository.save(tontineType);
  ...
  async findAll(actifOnly: boolean = false): Promise<TontineTypeResponseDto[]> {
    ...
    const types = await tontineTypeRepository.find({
      where,
      order: { libelle: 'ASC' },
    });
  ...
  async findById(id: string): Promise<TontineTypeResponseDto> {
    const type = await tontineTypeRepository.findOne({ where: { id } });
    if (!type) {
  ...
  async findByCode(code: string): Promise<TontineTypeResponseDto> {
    const type = await tontineTypeRepository.findOne({ where: { code } });
    if (!type) {
  ...
  async update(id: string, dto: UpdateTontineTypeDto): Promise<TontineTypeResponseDto> {
    ...
    const updated = await tontineTypeRepository.save(type);
